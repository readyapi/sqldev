...
Here I'll tell you how **SQLDev** interacts with the database, why you would want to use it (or use a similar tool), and how it relates to SQL.
...
It comes by default in **SQLDev** (thanks to SQLAlchemy). And many other similar tools would also provide that functionality among many other features.
...
With **SQLDev**, instead of writing SQL statements directly, you use Python classes and objects to interact with the database.
...
Then **SQLDev** will convert that to a literal string `"2; DROP TABLE hero"`.
...
Of course, there are also other ways to do SQL data sanitization without using a tool like **SQLDev**, but it's still a nice feature you get by default.
...
These types of libraries like **SQLDev** (and of course, SQLAlchemy) that translate between SQL and code with classes and objects are called **ORMs**.
...
There are many ORMs available apart from **SQLDev**, you can read more about some of them in [Alternatives, Inspiration and Comparisons](alternatives.md){.internal-link target=_blank}
...
Nevertheless, **SQLDev** and many other similar tools can generate a table name automatically from your code, as you will see later in the tutorial.
...
So, to keep things consistent, I'll keep using the same table names that **SQLDev** would have generated.
