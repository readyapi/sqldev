...
Remember that [Order Matters](create-db-and-table.md#sqldev-metadata-order-matters){.internal-link target=_blank} when calling `SQLDev.metadata.create_all()`?

The point of that section in the docs is that you have to import the module that has the models **before** calling `SQLDev.metadata.create_all()`.
...
Created hero: id=1 secret_name='Nx Pkg' team_id=1 name='Deadpond' age=None
...
Now we get editor support, autocompletion, inline errors, and **SQLDev** keeps working. 🎉
...
Created hero: id=1 age=None name='Deadpond' secret_name='Nx Pkg' team_id=1
