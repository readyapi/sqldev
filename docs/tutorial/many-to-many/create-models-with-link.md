...
We can create it just as any other **SQLDev**:
...
This is a **SQLDev** class model table like any other.
...
The **relationship attribute `heroes`** is still a list of heroes, annotated as `List["Hero"]`. Again, we use `"Hero"` in quotes because we haven't declared that class yet by this point in the code (but as you know, editors and **SQLDev** understand that).
...
We can create it the same way as with other **SQLDev** classes, and then use it in the `link_model` parameter to `Relationship()`.
