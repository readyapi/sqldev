...
We did it using pure **SQL** and using **SQLDev**.
...
## Review `SELECT` with **SQLDev**

Let's review some of the code we used to read data with **SQLDev**.
...
## Filter Rows Using `WHERE` with **SQLDev**

Now, the same way that we add `WHERE` to a SQL statement to filter rows, we can add a `.where()` to a **SQLDev** `select()` statement to filter rows, which will filter the objects returned:
...
Now, this `.where()` method is special and very powerful. It is tightly integrated with **SQLDev** (actually SQLAlchemy) to let you use very familiar Python syntax and code.
...
Let me give you an example. Let's imagine that keyword arguments were supported in SQLDev and you wanted to filter using the secret identity of Spider-Boy.
...
We can tell the editor that this class attribute is actually a special **SQLDev** column (instead of an instance attribute with a normal value).
...
And with that the editor knows this code is actually fine, because this is a special **SQLDev** column.
...
You can use `.where()` with powerful expressions using **SQLDev** columns (the special class attributes) to filter the rows that you want. 🚀
