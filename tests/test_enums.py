...
Associated issues:
* https://github.com/khulnasoft/sqldev/issues/96
* https://github.com/khulnasoft/sqldev/issues/164
"""
...
def test_postgres_ddl_sql(clear_sqldev, capsys: pytest.CaptureFixture[str]):
    assert test_enums_models, "Ensure the models are imported and registered"
    importlib.reload(test_enums_models)
    SQLDev.metadata.create_all(bind=postgres_engine, checkfirst=False)
...
def test_sqlite_ddl_sql(clear_sqldev, capsys: pytest.CaptureFixture[str]):
    assert test_enums_models, "Ensure the models are imported and registered"
    importlib.reload(test_enums_models)
    SQLDev.metadata.create_all(bind=sqlite_engine, checkfirst=False)
