...
class SQLDevMetaclass(ModelMetaclass, DeclarativeMeta):
    __sqldev_relationships__: Dict[str, RelationshipInfo]
    model_config: SQLDevConfig
    model_fields: Dict[str, FieldInfo]
    __config__: Type[SQLDevConfig]
    ...
    def __new__(
        ...
        **kwargs: Any,
        ...
        dict_used = {
            **dict_for_pydantic,
            "__weakref__": None,
            "__sqldev_relationships__": relationships,
    ...
    def __init__(
        ...
        if is_table_model_class(cls) and not base_is_table:
            for rel_name, rel_info in cls.__sqldev_relationships__.items():
...
_TSQLDev = TypeVar("_TSQLDev", bound="SQLDev")


class SQLDev(BaseModel, metaclass=SQLDevMetaclass, registry=default_registry):
    # SQLAlchemy needs to set weakref(s), Pydantic will set the other slots values
    __slots__ = ("__weakref__",)
    __tablename__: ClassVar[Union[str, Callable[..., str]]]
    __sqldev_relationships__: ClassVar[Dict[str, RelationshipProperty[Any]]]
    __name__: ClassVar[str]
    metadata: ClassVar[MetaData]
    __allow_unmapped__ = True  # https://docs.sqlalchemy.org/en/20/changelog/migration_20.html#migration-20-step-six

    if IS_PYDANTIC_V2:
        model_config = SQLDevConfig(from_attributes=True)
    else:
    ...
    def __init__(__pydantic_self__, **data: Any) -> None:
        ...
        if finish_init.get():
            sqldev_init(self=__pydantic_self__, data=data)
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if name in {"_sa_instance_state"}:
            ...
            if name not in self.__sqldev_relationships__:
                super().__setattr__(name, value)
    ...
    @deprecated(
        """
        🚨 `obj.dict()` was deprecated in SQLDev 0.0.14, you should
    ...
    @deprecated(
        """
        🚨 `obj.from_orm(data)` was deprecated in SQLDev 0.0.14, you should
        instead use `obj.model_validate(data)`.
        """
    )
    def from_orm(
        cls: Type[_TSQLDev], obj: Any, update: Optional[Dict[str, Any]] = None
    ) -> _TSQLDev:
    ...
    @deprecated(
        """
        🚨 `obj.parse_obj(data)` was deprecated in SQLDev 0.0.14, you should
        instead use `obj.model_validate(data)`.
        """
    )
    def parse_obj(
    ...
    def sqldev_update(
        ...
        *,
        ...
        if isinstance(obj, dict):
            ...
            raise ValueError(
                "Can't use sqldev_update() with something that "
                f"is not a dict or SQLDev or Pydantic model: {obj}"
            )
        return self
